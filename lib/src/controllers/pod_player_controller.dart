...
class PodPlayerController {
  ...
  void togglePlayPause() {
    ...
    logger.i('_ctr.videoPosition togglePlayPause ${_ctr.videoPosition}');
  }
  ...
  /// Returns the video progress as a double between 0 and 1.
  double getVideoProgress() {
    final totalDuration = _ctr.videoDuration.inMilliseconds;
    final currentPosition = _ctr.videoPosition.inMilliseconds;
    logger.i('totalDuration $totalDuration currentPosition $currentPosition');
    if (totalDuration == 0) {
  ...
  double getCurrentPositionOnSlider(double sliderValue) {
    final totalDuration = _ctr.videoDuration.inMilliseconds;
    if (totalDuration == 0) {
      return 0; // Avoid division by zero
    }

    final double currentPosition = sliderValue * totalDuration.toDouble();
