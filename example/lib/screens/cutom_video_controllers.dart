...
class _CustomVideoControllsState extends State<CustomVideoControlls> {
  ...
  double currentSliderPrimaryValue = 0.0;
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    sizeH20,
                    _iconButton('Show Pause', Icons.pause_circle,
                        onPressed: () {
                      controller.pause();
                    }),
                    sizeH20,
                    Listener(
                      onPointerMove: (PointerEvent event) {
                        if (event is PointerHoverEvent) {
                          RenderBox? renderBox = _sliderKey.currentContext!
                              .findRenderObject() as RenderBox?;
                          double sliderWidth = renderBox!.size.width;
                          // Calculate the new position based on the mouse movement
                          double mouseX = event.localPosition.dx;
                          double newPosition = mouseX / sliderWidth;

                          // Ensure newPosition is within the valid range (0.0 to 1.0)
                          newPosition = newPosition.clamp(0.0, 1.0);

                          // Calculate the corresponding video position
                          double videoPositionInSeconds = newPosition *
                              controller.currentVideoPosition.inSeconds;

                          // Update the slider position
                          controller.videoSeekTo(Duration(
                              seconds: videoPositionInSeconds.toInt()));
                        }
                      },
                      child: Slider(
                        key: _sliderKey,
                        value: controller.getVideoProgress(),
                        ...
                        onChangeEnd: (value) {
                          double currentPosition =
                              controller.getCurrentPositionOnSlider(value);
                          logger.i('Current Position: $currentPosition');
                          controller.videoSeekTo(
                              Duration(seconds: currentPosition.toInt()));
                          // Do something with the current position, like updating UI
                        },
                      ),
                    ),
                    sizeH20,
                    _iconButton('Show Play', Icons.play_arrow, onPressed: () {
                      controller.play();
                    }),
                    sizeH20,
